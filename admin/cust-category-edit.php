...
if (isset($_POST['form1'])) {
    ...
    $existing_cat = $pdo->prepare("SELECT * FROM tbl_cust_category WHERE cat_id=?");
    $existing_cat->execute(array($_REQUEST['id']));
    $total = $existing_cat->fetchAll();
    $cat_name = $total[0]['cat_name'];
    $cust_id = $total[0]['customer'];
    ...
    if ($valid == 1) {
        updateCategory($pdo, $ftpConnection, $cat_name, $_POST['cat_name'], $cat_type_name, $customer);
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-4">
                                <select name="cust_id" class="form-control select2 customer">
                                    ...
                                    <?php
                                    $statement = $pdo->prepare("SELECT * FROM tbl_user where role='Customer' ORDER BY full_name ASC");
                                    $statement->execute();
                                    $result = $statement->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($result as $row) {
                                    ?>
                                        <option value="<?php echo $row['id']; ?>" <?php if ($row['id'] == $id) {
                                                                                        echo 'selected';
